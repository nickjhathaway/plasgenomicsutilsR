...
web, so there is no need to keep a copy. Two released *P. falciparum*
3D7 annotations, both of which this package handles:
...
Both are versioned in the filename, so bump the release number when it
moves on. They name their attributes differently — PlasmoDB writes
`Parent=PF3D7_0709000.1` with a `gene_id`, Ensembl writes
`Parent=transcript:PF3D7_0709000.1` and no gene id at all — and
[`read_gff_cds()`](https://nickjhathaway.github.io/plasgenomicsutilsR/reference/read_gff_cds.md)
normalises both, deriving the gene id from the transcript when it has
to. They agree on the answers: *pfcrt* codon 76 comes out at
`403,624-403,626` either way.

Those chunks are not run when this page is built, so the site never
depends on the network. Everything below uses a small annotation written
out here instead, so the numbers can be checked by eye.
...
#> 1 A.1           A       chr1   1001  1005 +          0
#> 2 A.1           A       chr1   1101  1120 +          1
#> 3 B.1           B       chr2   2001  2030 -          0
```
...
#> # A tibble: 3 × 6
#>   name    chr   start   end codon_positions spans_intron
#>   <chr>   <chr> <dbl> <dbl> <chr>           <lgl>       
#> 1 A.1-AA1 1      1001  1003 1001,1002,1003  FALSE       
#> 2 A.1-AA2 1      1004  1101 1004,1005,1101  TRUE        
#> 3 A.1-AA3 1      1102  1104 1102,1103,1104  FALSE
```

Codon 1 is `1001,1002,1003` and codon 2 is `1004,1005,1101` — the first
exon runs out after five bases, so codon 2 straddles the intron. When
that happens `start` and `end` span the intron too, which makes the
interval 98 bp wide rather than 3, so `spans_intron` flags it and
...
On the minus strand the transcript is read from the highest coordinate
down:
...
#> # A tibble: 2 × 3
#>   name    codon_positions strand
#>   <chr>   <chr>           <chr> 
#> 1 B.1-AA1 2028,2029,2030  -     
#> 2 B.1-AA2 2025,2026,2027  -
```

`transcript_id` also accepts a gene id, or a gene symbol when `genes`
carries one — so with the bundled annotation `"pfcrt"` works as well as
`"PF3D7_0709000.1"`.
...
#> # A tibble: 5 × 5
#>     pos transcript_id aa_position codon_base coding
#>   <dbl> <chr>               <int>      <int> <lgl> 
#> 1  1001 A.1                     1          1 TRUE  
#> 2  1004 A.1                     2          1 TRUE  
#> 3  1101 A.1                     2          3 TRUE  
#> 4  1050 NA                     NA         NA FALSE 
#> 5  1121 NA                     NA         NA FALSE
```
...
reported, since those are the numbers people quote. `codon_base` says
which of the codon’s three bases the SNP is, in transcript orientation —
so `1004` and `1101` are bases 1 and 3 of the same codon 2. `1050` is
intronic and `1121` past the CDS, so both are non-coding and come back
`NA`; `keep = "hits"` drops them instead.

On a minus-strand gene, `codon_base == 1` is the *highest* genomic
...
the alleles and the codon’s other two bases, which neither function
does.
...
shifts ’s 1-based `POS` on the way in, so a scan built from those
genotypes lines up exactly with the IBD tables and with these codon
intervals. Nothing extra to do.
...
wrong is a silent one-base slide, enough to turn a near-miss into a hit:
