...
sNMF fits several replicates per K and scores each by cross-entropy. `q()` and
`plot_admixture()` use the **minimum**-cross-entropy replicate, so `min` is the column
describing the ancestry you actually see; `mean`/`max` show how far the replicates
disagreed. Picking K is a separate judgement — read the elbow, and treat a flat stretch or
a wide min–max band as "the data don't pin K down", whatever `best_k()` returns:
